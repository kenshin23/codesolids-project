...
import codesolids.gui.buzon.Buzon;
...
import codesolids.gui.ranking.RankingPersonaje;
...
public class MapaDesktop extends ContentPane {
	...
	private Component initMapa(){
		...
		ImageReference mensaje_des = ImageReferenceCache.getInstance().getImageReference("Images/Mapa/buzon_des.png");
		ImageReference mensajeNew_des = ImageReferenceCache.getInstance().getImageReference("Images/Mapa/buzonnuevo_des.png");
		...
		hld = new HtmlLayoutData("botonBuzon");

		Button btnBuzon = new Button();
		btnBuzon.setEnabled(true);
		btnBuzon.setVisible(true);
		btnBuzon.setRolloverEnabled(true);
		btnBuzon.setHeight(new Extent(47, Extent.PX));
		btnBuzon.setWidth(new Extent(47, Extent.PX));
		...
		if( validateMensaje() )
			btnBuzon.setBackgroundImage(new FillImage(mensajeNew_des));
		else	
			btnBuzon.setBackgroundImage(new FillImage(mensaje_des));
		
		btnBuzon.setLayoutData(hld);
		htmlLayout.add(btnBuzon);
	...
	private void btnBuzonClicked() {
		...
		if( msgList.isEmpty() )
		{
			...
			Column col = new Column();
			col.add(new Label("Usted no tiene mensanjes en el buzón."));

			MessageBox messageBox  = new MessageBox("Buzón de Mensajes",// 
					col,// 
					400, 130,//
	...
	private boolean validateMensaje()
	{
		...
		Criteria criteria = session.createCriteria(Mensaje.class).add(Restrictions.and(Restrictions.eq("personajeReceivesRef", personaje), Restrictions.eq("leido", false)));
		List<Mensaje> msgList = criteria.list();
	...
	private Column infoColumn() {
		...
		lbl = new Label();
		lbl.setForeground(Color.WHITE);		
		lbl.setText(personaje.getXp() + "/" + nivelExp.getCantidadExp());
		row.add(lbl);
		col.add(row);
		...
		lbl = new Label();
		lbl.setForeground(Color.WHITE);
		lbl.setText(personaje.getHp() + "/" + personaje.getHp());
		row.add(lbl);
		col.add(row);
		...
		lbl = new Label();
		lbl.setForeground(Color.WHITE);
		lbl.setText(personaje.getMp() + "/" + personaje.getMp());
		row.add(lbl);
		col.add(row);
